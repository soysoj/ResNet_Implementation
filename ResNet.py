...
def build_ResNet_block(input_layer, 
    ...
    if layer < 50:
        ...
        if stride != 1:
            skip = conv_block(filters = channel, kernel_size = (1,1), stride = stride)(skip)
    ...
    #따라서 밑의 부분은 50 이상일때의 구조이다.
        ...
        # check if it is bottle neck
        if stride != 1:
            skip = conv_block(filters = channel[2], kernel_size = (1,1), stride = stride, padding = 'same')(skip)
...
def build_ResNet(input_shape=(32,32,3),
    ...
    for i, (channel, num_block) in enumerate(zip(channel_list, num_block_list)):
        ...
        for j in range(num_block):
            # bottle neck block
            if i != 0 and j == 0:
                x = build_ResNet_block(x, filters = channel, stride = 2, layer = num_layer)
                continue
            x = build_ResNet_block(x, filters = channel, stride = 2, layer = num_layer)
